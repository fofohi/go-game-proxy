...
func main() {
	...
	for {
		...
		handleAServerConn2(stream)
	}
}
func handleAServerConn2(client io.ReadWriteCloser){
	c1, _ := rc4.NewCipher([]byte(pws))
	c2, _ := rc4.NewCipher([]byte(pws))
	...
	byteHeader := deCodereadSplitString2(client, pcTos, []byte("\r\n\r\n"))
	...
	if method == "CONNECT" {
		...
		server.Write(byteHeader[0:]) //最始的报头信息已经解密了，这里直接转发给web
	}
	//接下来得到的都是还没有解密的信息，进行解密转发
	go pcTos.encryptCopy2(server, client) //服务端收到的是密文，编码后就成了明文并传给web
	psToc.encryptCopy2(client, server)    //web发过来的是明文，编码后就成了密文，并传给客户端
...
func deCodereadSplitString2(r io.ReadWriter, coder *Rc4, delim []byte) []byte {
	var rs []byte
	lenth := len(delim)
	curByte := make([]byte, 1)
...
func (c *Rc4) encryptCopy2(dst io.ReadWriter, src io.ReadWriter) {
	buf := make([]byte, 4096)
	var err error
	n := 0
